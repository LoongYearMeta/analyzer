...
class ScatterTimeChart extends ASCIIChart {
  draw(data, options = {}) {
    ...
    const xRange = minMax(data.map(d => d.x)) || { min: 0, max: 1 };
    const xMin = options.xRange?.min || xRange.min;
    const xMax = options.xRange?.max || xRange.max;
...
class LineChart extends ASCIIChart {
  draw(data, labels, options = {}) {
    ...
    const dataRange = minMax(data) || { min: 0, max: 1 };
    const yMin = options.yRange?.min || dataRange.min;
    const yMax = options.yRange?.max || dataRange.max;
...
class BarChart extends ASCIIChart {
  draw(data, options = {}) {
    ...
    const valueRange = minMax(entries.map(e => e.value)) || { max: 0 };
    const labelRange = minMax(entries.map(e => e.label.toString().length)) || { max: 0 };
    const maxValue = valueRange.max;
    const maxLabelLen = labelRange.max;
...
<body>
  ...
  generateScatterScript(chart, index) {
    ...
    const xRange = minMax(data.map(d => d.x)) || { min: 0, max: 1 };
    const xMin = opts.xRange?.min || xRange.min;
    const xMax = opts.xRange?.max || xRange.max;
